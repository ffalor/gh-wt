...
func setupIntegrationTest(t *testing.T) *integrationTestSetup {
	// Build the binary
	binaryPath := filepath.Join(t.TempDir(), "gh-wt")

	cmd := exec.Command("go", "build", "-o", binaryPath, ".")
	cmd.Dir = "/home/wsl/github/gh-wt"
...
func (s *integrationTestSetup) cleanup() {
	// Cleanup is handled by t.Cleanup in the tests
}
...
func TestCompletionCommandSubcommandsIntegration(t *testing.T) {
	...
	t.Run("install subcommand help", func(t *testing.T) {
		...
		outputStr := string(output)
		assert.Contains(t, outputStr, "Automatically install")
		assert.Contains(t, outputStr, "completion script")
	})
	...
	t.Run("uninstall subcommand help", func(t *testing.T) {
		...
		outputStr := string(output)
		assert.Contains(t, outputStr, "Automatically uninstall")
		assert.Contains(t, outputStr, "completion script")
	})
}
...
func TestCompletionInstallUninstallIntegration(t *testing.T) {
	...
	t.Run("install creates completion file", func(t *testing.T) {
		...
		completionPath := filepath.Join(tmpHome, ".bash_completion.d", "gh-wt")
...
func TestCompletionVisibilityIntegration(t *testing.T) {
	...
	// Should show completion command
